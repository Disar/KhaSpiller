...
class SpiRandom
{
	...
	/**
	 * Create a new SpiRandom object.
	 * 
	 * @param	InitialSeed  The first seed of this SpiRandom object. If ignored, a random seed will be generated.
	 */
	...
	{
		return initialSeed = rangeBound(Std.int(Math.random() * SpiMath.MAX_VALUE_INT));
	}
	 ...
	 */
	public function int(Min:Int = 0, Max:Int = SpiMath.MAX_VALUE_INT, ?Excludes:Array<Int>):Int
	{
		if (Min == 0 && Max == SpiMath.MAX_VALUE_INT && Excludes == null)
		{
	 ...
	 * See this image: https://github.com/HaxeFlixel/spiller-demos/blob/dev/Performance/SpiRandom/normaldistribution.png
	 * 
	...
	public function getObject<T>(Objects:Array<T>, ?WeightsArray:Array<Float>, StartIndex:Int = 0, ?EndIndex:Null<Int>):T
	{
		...
		if (Objects.length != 0)
		{
			...
			StartIndex = Std.int(SpiMath.bound(StartIndex, 0, Objects.length - 1));
			EndIndex = Std.int(SpiMath.bound(EndIndex, 0, Objects.length - 1));
	 ...
	 * 
	 * @param   Min        An optional SpiColor representing the lower bounds for the generated color.
	 * @param   Max        An optional SpiColor representing the upper bounds for the generated color.
	 ...
	 * @return  A color value as a SpiColor.
	 */
	public function color(?Min:SpiColor, ?Max:SpiColor, ?Alpha:Null<Int>, GreyScale:Bool = false):SpiColor
	{
		var red:Int;
		var green:Int;
		var blue:Int;
		...
		return SpiColor.fromRGB(red, green, blue, alpha);
	}
	...
	{
		return Std.int(SpiMath.bound(Value, 1, MODULUS - 1));
	}
