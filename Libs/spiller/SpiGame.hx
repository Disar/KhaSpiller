...
class SpiGame
{
	 ...
	 */
	public SpiGame(int GameSizeX, int GameSizeY, Class<? extends SpiState> InitialState, float Zoom, int GameFramerate, int FlashFramerate, boolean UseSystemCursor, int ScaleMode)
	{
		...
		stage = new SpiGameStage((int) (GameSizeX * Zoom), (int) (GameSizeY * Zoom));

		// Basic stuff
		_accumulator = (int) step;
		_total = 0;
		_state = null;
	 ...
	 * @param UseSystemCursor Whether to use the default OS mouse pointer, or to use custom spiller ones.
	 */
	public SpiGame(int GameSizeX, int GameSizeY, Class<? extends SpiState> InitialState, float Zoom, int GameFramerate, int FlashFramerate, boolean UseSystemCursor)
	{
		this(GameSizeX, GameSizeY, InitialState, Zoom, GameFramerate, FlashFramerate, true, SpiCamera.STRETCH);
	}
	 ...
	 * @param FlashFramerate Sets the actual display framerate for Flash player (default is 30 times per second).
	 */
	public SpiGame(int GameSizeX, int GameSizeY, Class<? extends SpiState> InitialState, float Zoom, int GameFramerate, int FlashFramerate)
	{
		this(GameSizeX, GameSizeY, InitialState, Zoom, GameFramerate, FlashFramerate, true, SpiCamera.STRETCH);
	}
	 ...
	 * @param GameFramerate How frequently the game should update (default is 30 times per second).
	 */
	public SpiGame(int GameSizeX, int GameSizeY, Class<? extends SpiState> InitialState, float Zoom, int GameFramerate)
	{
		this(GameSizeX, GameSizeY, InitialState, Zoom, GameFramerate, 60, true, SpiCamera.STRETCH);
	}
	 ...
	 * @param Zoom The default level of zoom for the game's cameras (e.g. 2 = all pixels are now drawn at 2x). Default = 1.
	 */
	public SpiGame(int GameSizeX, int GameSizeY, Class<? extends SpiState> InitialState, float Zoom)
	{
		this(GameSizeX, GameSizeY, InitialState, Zoom, 60, 60, true, SpiCamera.STRETCH);
	}
	 ...
	 * @param InitialState The class name of the state you want to create and switch to first (e.g. MenuState).
	 */
	public SpiGame(int GameSizeX, int GameSizeY, Class<? extends SpiState> InitialState)
	{
		this(GameSizeX, GameSizeY, InitialState, 1, 60, 60, true, SpiCamera.STRETCH);
	}
	...
	public void create()
	{
		...
		FileTextureData.copyToPOT = true; // Enable copy the textures to a power of two file.
