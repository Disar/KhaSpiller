...
class SpiStringUtil
{
	...
	public static function formatBytes(Bytes:Float, Precision:Int = 2):String
	{
		...
		return SpiMath.roundDecimal(Bytes, Precision) + units[curUnit];
	}
	 ...
	 * @return	A comma-separated string containing the level data in a SpiTilemap-friendly format.
	 */
	...
	//  * 
	//  * @param	canvas		A Flash Canvas object, preferably black and white.
	...
	//  * @return	A comma-separated string containing the level data in a SpiTilemap-friendly format.
	//  */
	// public static function bitmapToCSV(canvas:Canvas, Invert:Bool = false, Scale:Int = 1, ?ColorMap:Array<SpiColor>):String
	// {
	// 	if (Scale < 1) 
	// 	{
	// 		Scale = 1;
	...
	// 					for (m in 0...Scale)
	// 					{
	// 						canvas.setPixel(i * Scale + k, j * Scale + m, pCol);
	// 					}
	// 				}
	// 			}
	// 		}
	...
	//  * @return	A comma-separated string containing the level data in a SpiTilemap-friendly format.
	//  */
	// public static function imageToCSV(ImageFile:Dynamic, Invert:Bool = false, Scale:Int = 1, ?ColorMap:Array<SpiColor>):String
	// {
	// 	var tempCanvas:Canvas;
		
	// 	if (Std.is(ImageFile, String))
	// 	{
	// 		tempBitmapData = SpiAssets.getBitmapData(ImageFile);
	// 	}
	// 	else
	// 	{
	...
	/**
	 * Helper function to create a string for toString() functions. Automatically rounds values according to SpiG.debugger.precision.
	...
	public static function getDebugString(LabelValuePairs:Array<LabelValuePair>):String
	{
		...
		for (pair in LabelValuePairs)
		{
			...
			{
				//TODO value = SpiMath.roundDecimal(cast value, SpiG.debugger.precision);
			}
			output += (value + " | ");
...
@:allow(spiller.util.LabelValuePairPool)
class LabelValuePair implements ISpiDestroyable
{
	private static var _pool:LabelValuePairPool = new LabelValuePairPool();
	
	public static inline function weak(label:String, value:Dynamic):LabelValuePair
	{
		return _pool.getNew().create(label, value);
	}
...
class LabelValuePairPool extends SpiPool<LabelValuePair>
{
	override
	public function create():LabelValuePair
	{
